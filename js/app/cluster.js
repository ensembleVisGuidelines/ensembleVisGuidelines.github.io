define(['jquery', 'app/util', 'figue', 'app/selectors', 'app/bib'], function ($, util, figue, selectors, bib) {
    ...
    return {
        ...
        createClustering: function () {
            ...
            var clusterDataType = $('#datatype_checkbox').is(':checked');

            if (!clusterKeywords && !clusterAuthor && !clusterDataType) {
            ...
            $.each(bib.filteredEntries, function (id, entry) {
                ...
                if (clusterDataType && bib.parsedEntries[id].data_type) {
                    terms = terms.concat(bib.parsedEntries[id].data_type)
                }
                //if (terms.length > 0) {
